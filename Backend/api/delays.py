...
async def get_delay_report() -> dict[str, Any]:
    ...
    RETURN t.train_number AS train_number,
           ...
           collect(DISTINCT e.location)[0..5] AS event_locations
    ...
    for r in result:
        ...
        trains.append({
            ...
            "location": locs[0] if locs else r["current_track"],
            "reason": "Schedule disruption",  # Fallback — add 'reason' to Event nodes if you want real data
        })
    ...
    return {
        ...
        "avg_delay_minutes": round(total_delay / len(trains), 1) if trains else 0,  # Renamed from average_delay_minutes
        "affected_locations": sorted(list(all_locations)),  # Added missing field
    }
