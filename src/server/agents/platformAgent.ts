...
export class PlatformAgent {
  ...
  async evaluate(train: AgentTrain): Promise<(PlatformDecision | WaitDecision) | null> {
    ...
    // Skip junctions (no platforms)
    const junctions = ["J_NW","J_NC","J_NE","J_CW","J_CN","J_CE","J_MW","J_MC","J_SW","J_SC"];
    if (junctions.includes(nextStation)) return null;

    // Don't re-assign if already assigned to this station
    if (train.assigned_platform && train.current_station === nextStation) return null;
    ...
    if (availablePlatforms.length === 0) {
      return {
        ...
        reason: `No compatible platform at ${nextStation} for train ${train.train_id} (${train.length_meters}m)`,
        retry_in_seconds: 10,
      };
    }
    ...
    const dwellSeconds = Math.floor(Math.random() * 16) + 15; // 15–30 s

    logger.info(
      { trainId: train.train_id, station: nextStation, platform: best.number, score: best.total_score.toFixed(2) },
    ...
    return {
      ...
      reason: `Platform ${best.number} scored best (${best.total_score.toFixed(2)}) — ` +
        `wait: ${best.score_breakdown.waiting_time_score.toFixed(1)}, ` +
        `cong: ${best.score_breakdown.congestion_score.toFixed(1)}, ` +
        `len: ${best.score_breakdown.length_compatibility.toFixed(1)}`,
    };
  }
